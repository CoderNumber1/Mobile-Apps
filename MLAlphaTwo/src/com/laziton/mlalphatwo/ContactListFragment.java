...
public class ContactListFragment extends ListFragment {
    private ArrayList<Movie> contacts;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        dataService.Open();
        contacts = dataService.GetMovies();
        dataService.Close();
    ...
    public void onListItemClick(ListView listView, View view, int position, long id) {
        Movie contact = ((ContactsAdapter)getListAdapter()).getItem(position);
        Intent i = new Intent(getActivity(), ContactPagerActivity.class);
        i.putExtra(ContactFragment.MOVIE_ID, contact.getId());
	...
	public boolean onContextItemSelected(MenuItem item) {
		...
		Movie contact = adapter.getItem(position);
		
		switch(item.getItemId()){
			case R.id.menu_item_delete_contact:
				IDataService dataService = DataServiceFactory.GetInstance().GetDataService();
				dataService.Open();
				dataService.DeleteMovie(contact);
				contacts = dataService.GetMovies();
				dataService.Close();
				adapter.notifyDataSetChanged();
				return true;
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            case R.id.menu_item_new_contact:
                ...
                dataService.InsertMovie(contact);
                
                Intent i = new Intent(getActivity(), ContactActivity.class);
                Log.w("NewMovie", String.valueOf(contact.getId()));
                i.putExtra(ContactFragment.MOVIE_ID, contact.getId());
                startActivityForResult(i, 0);
                dataService.Close();
                return true;
            case R.id.menu_item_new_genre:
            	Genre genre = new Genre();
            	dataService.InsertGenre(genre);
            	dataService.Close();
            	Intent genreIntent = new Intent(getActivity(), GenreActivity.class);
            	Log.w("NewGenre", String.valueOf(genre.getId()));
            	genreIntent.putExtra(GenreFragment.GENRE_ID, genre.getId());
            	startActivityForResult(genreIntent,1);
            	dataService.Close();
            	return true;
// genreIntent     case R.id.menu_item_sort_name:
...
//            	((ContactsAdapter)getListAdapter()).notifyDataSetChanged();
	...
	private class ContactsAdapter extends ArrayAdapter<Movie> {
        public ContactsAdapter(ArrayList<Movie> contacts) {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            Movie contact = getItem(position);
